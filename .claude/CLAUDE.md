...
## 1. 🍳 API 설계
- 지정한 기능을 구성하기 전 PRD와 ERD 문서를 활용하여 해당 기능을 구성하기 위해 필요한 API 설계 문서를 docs/api 하위 경로에 작성합니다.
- API 설계는 도메인에 따라 서로 다른 파일에 작성합니다.
- API 설계 문서에는 구성하고자 하는 기능 개요, 연관 테이블, 레이어드 간 통신을 표현한 시퀀스 다이어그램, API 엔드포인트, 처리하는 Exception 을 표현합니다.
- 시퀀스 다이어그램은 DB 쿼리나 메서드와 같이 명령적으로 작성하지 않고, 행위 중심으로 표현합니다. 또한 트랜잭션의 범위와 롤백 플랜을 올바르게 명시합니다.
