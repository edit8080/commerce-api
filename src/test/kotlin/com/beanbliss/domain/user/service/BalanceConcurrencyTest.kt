...
class BalanceConcurrencyTest : ConcurrencyTestBase() {
    ...
    fun `Lost Update_동시 충전`() {
        ...
        assertEquals(expectedAmount, finalAmount, "잔액 충전이 정확히 반영되어야 합니다")
    }
    ...
    fun `Lost Update_동시 차감`() {
        ...
        assertEquals(expectedAmount, finalAmount, "잔액 차감이 정확히 반영되어야 합니다")
    }
    ...
    fun `혼합_동시 충전과 차감`() {
        ...
        assertEquals(expectedAmount, finalAmount, "충전과 차감이 모두 정확히 반영되어야 합니다")
    }
    ...
    fun `잔액 부족_동시 차감`() {
        ...
        repeat(concurrentUsers) {
            executor.submit {
                try {
                    balanceService.deductBalance(testUserId, deductAmount)
                    successCount.incrementAndGet()
                } catch (e: com.beanbliss.domain.user.exception.InsufficientBalanceException) {
        ...
        assertEquals(BigDecimal.ZERO, finalAmount, "최종 잔액은 0원이어야 합니다")
    }
}
